...
case class stepTransient(targetNum: Long, srcRDD: RDD[(Long, Double)], sc: SparkContext, partitionNum: Int) extends Algorithm with AlgoAction {
  override def execute(cacheable: Boolean, additionalParamMap: mutable.Map[String, Int]): RDD[(Long, Double)] = {
    ...
    if (!additionalParamMap.isEmpty) {
      m1 = additionalParamMap.getOrElse("m1", 500)
      m2 = additionalParamMap.getOrElse("m2", 2)
    }

    val totalLen: Long = zipRDD.count()
    val sectionLen: Long = totalLen / m1
    val T2 = targetNum / m1
